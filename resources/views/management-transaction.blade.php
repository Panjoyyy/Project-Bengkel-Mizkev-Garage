...
@section('content')
    ...
    @forelse($transaksi as $item)
        <tr data-aos="fade-up" data-aos-delay="{{ $loop->iteration * 40 }}">
            ...
            <!-- Layanan -->
            <td>
                @foreach($item->getLayananData() as $lay)
                    <span class="badge bg-primary mb-1">{{ $lay->nama_layanan }}</span><br>
                @endforeach
            </td>
            <!-- Sparepart -->
            <td>
                @foreach($item->getSparepartData() as $sp)
                    <span class="badge bg-secondary mb-1">
                        {{ $sp->nama_sparepart }} (x{{ $sp->jumlah_beli }})
                    </span><br>
                @endforeach
            </td>
