        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 style="font-size: 1.8rem; font-weight: 700; color: #1a2332; margin-bottom: 5px;">
                    <i class="fas fa-users" style="color: #1a2332; margin-right: 10px;"></i>{{ $title }}
                </h2>
                <p style="color: #6c757d; margin: 0; font-size: 0.95rem;">Kelola semua data customer bengkel</p>
            </div>
            <a href="{{ route('create-customer-form') }}" class="btn-success-custom" style="text-decoration: none;">
                <i class="fas fa-plus-circle me-2"></i>Tambah Customer
            </a>
        </div>

        <!-- Search Bar -->
        <form action="{{ route('management-customer') }}" method="GET" class="mb-4">
        ...
        <table class="table-modern">
            ...
            <tbody>
                ...
                <tr data-aos="fade-up" data-aos-delay="{{ $loop->iteration * 50 }}">
                    ...
                    <td style="font-family: monospace; color: #1a2332; font-weight: 600;">#{{ $item->id_customer }}</td>
                    <td style="font-weight: 600; color: #1a2332; font-size: 1rem;">{{ $item->nama_customer }}</td>
                    <td>
                        <span style="background: #f3f4f6; color: #1a2332; padding: 6px 12px; border-radius: 8px; font-weight: 500; font-size: 0.9rem; display: inline-block;">
                            <i class="fas fa-phone me-2" style="color: #10b981;"></i>{{ $item->no_telp_customer }}
                        </span>
                    </td>
                    <td style="color: #6c757d; font-size: 0.9rem;">{{ Str::limit($item->alamat_customer, 40) }}</td>
                    <td style="color: #6c757d; font-size: 0.9rem;">
                        <i class="fas fa-envelope me-2" style="color: #3b82f6;"></i>{{ $item->email_customer }}
                    </td>
                    <td>
                        <div class="d-flex gap-2 justify-content-center">
                            <a href="{{ route('edit-customer-form', $item->id_customer) }}" 
                            ...
                            <button class="btn-danger-custom" 
                                    ...
                                    data-bs-target="#delete{{ $item->id_customer }}"
                                    title="Hapus">
                ...
                <tr>
                    <td colspan="7" style="text-align: center; padding: 60px 20px;">
                        <i class="fas fa-users" style="font-size: 4rem; color: #1a2332; margin-bottom: 20px;"></i>
                        <p style="color: #6c757d; font-size: 1.1rem; margin: 0;">Tidak ada data customer</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
...
<div class="modal fade" id="delete{{ $item->id_customer }}" tabindex="-1" aria-labelledby="deleteLabel{{ $item->id_customer }}" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 20px; border: none; box-shadow: 0 10px 40px rgba(0,0,0,0.2);">
            <div class="modal-header" style="border: none; padding: 30px 30px 0;">
                <h5 class="modal-title" id="deleteLabel{{ $item->id_customer }}" style="color: #1a2332; font-weight: 700;">
            ...
            <form action="{{ route('delete-customer', $item->id_customer) }}" method="POST">
                ...
                <div class="modal-body" style="padding: 20px 30px;">
                    <p style="color: #6c757d; margin: 0;">
                        Apakah Anda yakin ingin menghapus customer <strong style="color: #1a2332;">{{ $item->nama_customer }}</strong>?
                    </p>
